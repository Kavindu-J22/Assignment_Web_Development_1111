...
try {
    ...
    // Validate rating (optional, but if provided should be 1-5)
    ...
    // Save rating separately if provided
    if ($rating > 0) {
        saveRating($rating, $email, $contactData['id']);
        logActivity("New rating: {$rating} stars from {$email}");
    }

    // Log the contact submission
    logActivity("New contact message from: {$email} - Subject: {$subject}" . ($rating > 0 ? " - Rating: {$rating} stars" : ""));
